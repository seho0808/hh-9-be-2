...
- `rows: 180` - is_active 조건으로 필터링 후에도 많은 행 검사
...
- ✅ **완벽한 인덱스 활용**: `idx_orders_user_created` 복합 인덱스 사용
- ✅ **filesort 제거**: ORDER BY가 인덱스 순서로 처리되어 정렬 작업 불필요
- ✅ **JOIN 최적화**: order_items에서도 정상적인 인덱스 사용
...
#### 실패한 주문 배치 조회 (ANALYZE TABLE 후 인덱스 적용)
...
- ✅ **filesort 제거**: ORDER BY updated_at이 인덱스로 처리
- ✅ **효율적인 조인**: order_items도 인덱스 정상 사용
