  ...
  describe("ReserveStockUseCase (@Transactional)", () => {
    it("재고 예약이 성공적으로 처리되어야 함", async () => {
      ...
      const savedReservation = await stockReservationRepository.findOne({
        where: { idempotencyKey: "reserve-key-1" },
      });
  ...
  describe("ReleaseStockUseCase (@Transactional)", () => {
    it("재고 해제가 성공적으로 처리되어야 함", async () => {
      ...
      const command = {
        ...
        idempotencyKey: stockReservation.idempotencyKey,
      };
      ...
      const savedReservation = await stockReservationRepository.findOne({
        where: { idempotencyKey: stockReservation.idempotencyKey },
      });
        ...
        requests: [
          {
            ...
            userId: "user-123",
            quantity: 10,
            idempotencyKey: "batch-reserve-1",
          },
          {
            productId: product2.id,
            userId: "user-123",
            quantity: 5,
            idempotencyKey: "batch-reserve-2",
          },
        ],
      };
        ...
        requests: [
          {
            ...
            userId: "user-123",
            quantity: 10,
            idempotencyKey: "batch-reserve-fail-1",
          },
          {
            productId: product2.id,
            userId: "user-123",
            quantity: 5, // 가용 재고(2)보다 많음
            idempotencyKey: "batch-reserve-fail-2",
          },
        ],
      };
