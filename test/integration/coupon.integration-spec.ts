...
describe("Coupon Domain Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    // Create module with Coupon module for integration test
  ...
  describe("UseUserCouponUseCase (@Transactional)", () => {
    it("쿠폰 사용이 성공적으로 처리되어야 함", async () => {
      ...
      // Then: 쿠폰이 사용되고 할인이 적용되어야 함
      expect(result.coupon.id).toBe(coupon.id);
      expect(result.userCoupon.status).toBe("USED");
      expect(result.discountPrice).toBe(1000);
      expect(result.discountedPrice).toBe(4000); // 5000 - 1000
      ...
      expect(updatedUserCoupon.orderId).toBe("order-123");
      expect(updatedUserCoupon.discountPrice).toBe(1000);
      expect(updatedUserCoupon.usedAt).toBeDefined();
    });
    ...
    it("퍼센트 할인 쿠폰이 정확히 계산되어야 함", async () => {
      ...
      const command = {
        ...
        orderId: "order-456",
        orderPrice: 10000,
        idempotencyKey: "use-percent-coupon-key",
      };

      const result = await useUserCouponUseCase.execute(command);

      // Then: 20% 할인이 적용되어야 함
      expect(result.discountPrice).toBe(2000); // 10000 * 20%
      expect(result.discountedPrice).toBe(8000); // 10000 - 2000
    });
  ...
  describe("CancelUserCouponUseCase (@Transactional)", () => {
    it("쿠폰 취소가 성공적으로 처리되어야 함", async () => {
      ...
      // Then: 쿠폰이 취소 상태로 변경되어야 함
      expect(result.coupon.id).toBe(coupon.id);
      expect(result.userCoupon.status).toBe("CANCELLED");

      // DB 검증
      const updatedUserCoupon = await userCouponRepository.findOne({
        where: { id: userCoupon.id },
      });
      expect(updatedUserCoupon.status).toBe(UserCouponStatus.CANCELLED);
    });

    it("존재하지 않는 사용자 쿠폰에 대해 예외가 발생해야 함", async () => {
      // Given: 존재하지 않는 사용자 쿠폰 ID

      // When & Then: 사용자 쿠폰을 찾을 수 없음으로 예외 발생
      const command = {
        userCouponId: "non-existent-user-coupon",
      };
    ...
    it("이미 사용된 쿠폰은 취소할 수 없어야 함", async () => {
      // Given: 이미 사용된 사용자 쿠폰
      const coupon = await CouponFactory.createAndSave(couponRepository, {
        couponCode: "USED_CANNOT_CANCEL",
        discountValue: 1000,
        totalCount: 100,
        usedCount: 0,
      });
  ...
  describe("Coupon Lifecycle Integration", () => {
    it("쿠폰의 전체 생명주기가 올바르게 관리되어야 함", async () => {
      ...
      expect(issueResult.userCoupon.status).toBe("ISSUED");
      ...
      expect(useResult.userCoupon.status).toBe("USED");
      expect(useResult.discountPrice).toBe(1000);

      // Final DB 검증
      const finalUserCoupon = await userCouponRepository.findOne({
        where: { id: issueResult.userCoupon.id },
      });
      expect(finalUserCoupon.status).toBe(UserCouponStatus.USED);
      expect(finalUserCoupon.orderId).toBe("lifecycle-order");
      expect(finalUserCoupon.discountPrice).toBe(1000);
      expect(finalUserCoupon.usedAt).toBeDefined();

      // 쿠폰 사용량 증가 확인
      const finalCoupon = await couponRepository.findOne({
        where: { id: coupon.id },
      });
      expect(finalCoupon.issuedCount).toBe(1);
    });
  });
});
